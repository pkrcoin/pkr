...
uint256 hashGenesisBlock("0x65c6f7aac2c1e82899e0c0e375dedb163c140695e918d90399538e526f61f9ef");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // PKR coin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "PKR coin Signed Message:\n";
...
{
    // PKR coin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    int64 nSubsidy = 1 * COIN;
		if(nHeight == 1)
        {
                nSubsidy = 1448160 * COIN;
        }
		else if(nHeight <= 961)
        {
                nSubsidy = 35 * COIN;
        }
        else if(nHeight <= 4801)
		{
				nSubsidy = 350 * COIN;
		}
        else if(nHeight <= 10561)
        {
                nSubsidy = 343 * COIN;
        }
        else if(nHeight <= 18241)
        {
                nSubsidy = 333 * COIN;
        }
        else if(nHeight <= 27841)
        {
                nSubsidy = 320 * COIN;
        }
        else if(nHeight <= 39361)
        {
                nSubsidy = 304 * COIN;
        }
        else if(nHeight <= 52801)
        {
                nSubsidy = 285 * COIN;
        }
        
        else if(nHeight <= 72001)
        {
                nSubsidy = 265 * COIN;
        }
        else if(nHeight <= 96961)
        {
                nSubsidy = 239 * COIN;
        }
        else if(nHeight <= 127681)
        {
                nSubsidy = 208 * COIN;
        }
        else if(nHeight <= 164161)
        {
                nSubsidy = 175 * COIN;
        }
        else if(nHeight <= 204481)
        {
                nSubsidy = 142 * COIN;
        }
        else if(nHeight <= 250561)
        {
                nSubsidy = 112 * COIN;
        }
        
        else if(nHeight <= 310081)
        {
                nSubsidy = 85 * COIN;
        }
        else if(nHeight <= 383041)
        {
                nSubsidy = 59 * COIN;
        }
        else if(nHeight <= 469441)
        {
                nSubsidy = 37 * COIN;
        }
        else if(nHeight <= 569281)
        {
                nSubsidy = 20 * COIN;
        }
        
        else if(nHeight <= 669121)
        {
                nSubsidy = 10 * COIN;
        }
        else if(nHeight <= 768961)
        {
                nSubsidy = 5 * COIN;
        }
        else if(nHeight <= 868801)
        {
                nSubsidy = 2.5 * COIN;
        }
        else if(nHeight <= 968641)
        {
                nSubsidy = 1.25 * COIN;
        }
        else if(nHeight <= 1068481)
        {
                nSubsidy = 0.625 * COIN;
        }
        else if(nHeight <= 1168321)
        {
...
static const int64 nTargetSpacing = 90; // PKR coin: 1.5 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // PKR coin: This fixes an issue where a 51% attack can change difficulty at will.
...
{
        static const int64        BlocksTargetSpacing                        = 90; // 1.5 Minute
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // PKR coin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // PKR coin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x766ca234c8b72ee240ac4b63aa61b26171dfe9ee0d58cdc47b6b9f372f849b6e");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "3 february 2014";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1391525110;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 389309893;

        if (fTestNet)
        {
            block.nTime    = 1391463719;
            block.nNonce   = 385557369;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x1d7eaf1fc36da0d6614e9bf19794c1341207f7470c613acc6a98c3b41462ac1b"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // PKR coin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PKRcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PKRcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PKRcoinMiner(CWallet *pwallet)
{
    printf("PKRcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("pkrcoin-miner");
    ...
    try { loop {
        ...
        printf("Running PKRcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PKRcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PKRcoinMiner, pwallet));
}
